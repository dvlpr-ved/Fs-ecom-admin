<template>
            ...
            <div class="card">
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            <Button label="New Listing" icon="pi pi-plus" class="mr-2" severity="success" @click="openNew" />
                            <Button label="New Banner" icon="pi pi-plus" class="mr-2" severity="success" @click="openNewBanner" />
                            <Button label="Manage Sequence" icon="pi pi-cog" class="mr-2" severity="success" @click="showSequenceDialog" />
                        </div>
                    </template>
                </Toolbar>
                <DataTable
                    ...
                    <Column field="Action" header="Action" headerStyle="min-width:10rem;">
                        <template #body="slotProps">
                            <Button v-if="!slotProps.data.is_banner" icon="pi pi-pencil" class="mr-2" severity="success" rounded @click="editEntryOpen(slotProps.data)" />
                            <Button v-else icon="pi pi-pencil" class="mr-2" severity="success" rounded @click="editEntryOpenBanner(slotProps.data)" />
                            <Button icon="pi pi-trash" class="mt-2" severity="warning" rounded @click="confirmDeleteEntry(slotProps.data)" />
                        </template>
                    </Column>
                </DataTable>

                <Dialog v-model:visible="createDialog" :style="{ width: '450px' }" header="Create Listing" :modal="true" class="p-fluid">
                    <div class="field">
                ...
                <Dialog v-model:visible="editDialog" :style="{ width: '450px' }" header="Edit Category" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Name</label>
                        <InputText id="name" v-model.trim="editEntry.name" required="true" autofocus :invalid="submittededit && !editEntry.name" />
                        <small class="p-invalid" v-if="submittededit && !editEntry.name">Name is required.</small>
                    </div>
                    <template #footer>
                ...
                <!-- Banner -->
                <Dialog v-model:visible="createDialogBanner" :style="{ width: '450px' }" header="Create banner" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Name</label>
                        <InputText id="name" v-model.trim="makeEntryBanner.name" required="true" autofocus :invalid="submitted && !makeEntryBanner.name" />
                        <small class="p-invalid" v-if="submittedBanner && !makeEntryBanner.name">Name is required.</small>
                    </div>
                    <div class="field col">
                        <label for="name">Images</label>
                        <Button  icon="pi pi-upload" class="ml-2" rounded severity="success"  @click="imageUploaderBanner = true" />
                        <span class="text-primary"></span>
                        {{ makeEntryBanner.image.length ? makeEntryBanner.image.length + ' Selected' : '' }}
                    </div>               
                    <ImageUploader :visible="imageUploaderBanner" v-model="makeEntryBanner.image" @close="imageUploaderBanner = false" ></ImageUploader>
                    <template #footer>
                        <Button label="Cancel" icon="pi pi-times" class="mr-2" severity="danger"  @click="hideDialogBanner" />
                        <Button label="Save" icon="pi pi-check" class="mr-2" severity="success"  @click="saveEntryBanner" />
                    </template>
                </Dialog>

                <Dialog v-model:visible="editDialogBanner" :style="{ width: '450px' }" header="Edit banner" :modal="true" class="p-fluid">
                    <div class="field">
                        <label for="name">Name</label>
                        <InputText id="name" v-model.trim="editEntryBanner.name" required="true" autofocus :invalid="submittededitBanner && !editEntryBanner.name" />
                        <small class="p-invalid" v-if="submittededitBanner && !editEntryBanner.name">Name is required.</small>
                    </div>
                    <div class="field col">
                        <label for="name">Images</label>
                        <Button  icon="pi pi-upload" class="ml-2" rounded severity="success"  @click="imageUploaderBannerEdit = true" />
                        <span class="text-primary"></span>
                        {{ editEntryBanner.image.length ? editEntryBanner.image.length + ' Selected' : '' }}
                    </div>
                    <ImageUploader :visible="imageUploaderBannerEdit" v-model="editEntryBanner.image" @close="imageUploaderBannerEdit = false" ></ImageUploader>
                    <template #footer>
                        <Button label="Cancel" icon="pi pi-times" class="mr-2" severity="danger"  @click="hideDialogEdit" />
                        <Button label="Save" icon="pi pi-check" class="mr-2" severity="success"  @click="updateEntryBannerEdit" />
                    </template>
                </Dialog>      
    ...
    <Dialog v-model:visible="showSequence" :style="{ width: '50%' }" position="top" header="Manage sequence" :modal="true" class="p-fluid">
        <div class="feild grid mt-2">
            <div v-for="(listin , index) in sequence_data"  class="col-12 flex">
                <div class="field col-6">
                    <label><b>{{ listin.name }}</b></label>
                </div>
                <div class="field col-6">
                    <InputNumber v-model="sequence_data[index].sequence"  required="true" autofocus />
                </div>
            </div>
        </div>
        <template #footer>
            <Button label="Cancel" icon="pi pi-times" class="mr-2" severity="danger"  @click="showSequence = false" />
            <Button label="Save" icon="pi pi-check" class="mr-2" severity="success"  @click="saveEntrySequence" />
        </template>        
    </Dialog>
</template>
...
const saveEntry = async () => {
    ...
    if(makeEntry.name != ''){
        ...
        if(res.success){
            createDialog.value= false;
            getTableData();
            toast.add({ severity: 'success', summary: 'Lising added successfully', life: 3000 });
        }
        else{
...
const updateEntry =async () => {
    ...
    if(editEntry.name != ''){
        ...
        if(res.success){
            editDialog.value= false;
            getTableData();
            toast.add({ severity: 'success', summary: 'Listing updated successfully', life: 3000 });
        }
        else{
            toast.add({ severity: 'error', summary: 'Something went wrong', life: 3000 });
        }
...
const deleteEntrySingle =async () => {
    ...
    if(res.success){
        ...
        getTableData();
        toast.add({ severity: 'success', summary: 'Listing deleted successfully', life: 3000 });
    }
    else{
        toast.add({ severity: 'error', summary: 'Something went wrong', life: 3000 });
    }
...
const saveEntryBanner = async () => {
    ...
    if(makeEntryBanner.name != ''){
        ...
        const res = await makeCustomRequest({
            url : url,
            method : 'POST',
            body : {name : makeEntryBanner.name , is_banner : makeEntryBanner.is_banner , images : makeEntryBanner.image}
        });
        if(res.success){
            createDialogBanner.value= false;
            getTableData();
...
const updateEntryBannerEdit  =async () => {
    ...
    if(editEntryBanner.name != ''){
        ...
        const res = await makeCustomRequest({
            url : url,
            method : 'PATCH',
            body : {name : editEntryBanner.name ,  is_banner : editEntryBanner.is_banner , images : editEntryBanner.image}
        });
        if(res.success){
            editDialogBanner.value= false;
            getTableData();
            toast.add({ severity: 'success', summary: 'Banner updated successfully', life: 3000 });
        }
        else{
            toast.add({ severity: 'error', summary: 'Something went wrong', life: 3000 });
        }
...
const saveEntrySequence =async () => {
   ...
   const res = await makeCustomRequest({
    ...
    if(res.success){
        toast.add({ severity: 'success', summary: 'Sequence updated successfully', life: 3000 });
