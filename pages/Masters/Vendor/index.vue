        ...
        <div class="col-12">
            <div class="card">
                ...
                <DataTable
                    ...
                    <Column field="Address" header="Address"  headerStyle="width:auto; min-width:10rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">Address</span>
                            {{ slotProps.data.address }}
                        </template>
                    </Column>
                    <Column field="Created at" header="Created at"  headerStyle="width:auto; min-width:10rem;">
                        <template #body="slotProps">
                ...
                <Dialog v-model:visible="createDialog" :style="{ width: '450px' }" header="Create vendor" :modal="true" class="p-fluid">
                    <div class="field">
                    ...
                    <div class="field">
                        ...
                        <InputText id="name" v-model.trim="makeEntry.address" required="true" autofocus :invalid="submitted && !makeEntry.address" />
                    </div>
                    <template #footer>
                ...
                <Dialog v-model:visible="editDialog" :style="{ width: '450px' }" header="Edit vendor" :modal="true" class="p-fluid">
                    <div class="field">
                    ...
                    <div class="field">
                        <label for="name">Address</label>
                        <InputText id="name" v-model.trim="editEntry.address" required="true" autofocus :invalid="submittededit && !editEntry.address" />
                    </div>
                    <template #footer>
...
const saveEntry = async () => {
    ...
    if(makeEntry.name != ''){
        const url = "api/Masters/Vendor";
        const res = await makeCustomRequest({
            url : url,
            method : 'POST',
            body : {name : makeEntry.name, address : makeEntry.address }
        });
        if(res.success){
            createDialog.value= false;
            getTableData();
            toast.add({ severity: 'success', summary: 'Vendor added successfully', life: 3000 });
        }
        else{
...
const updateEntry =async () => {
    ...
    if(editEntry.name != ''){
        const url = `api/Masters/Vendor/${editEntry.id}`;
        const token = localStorage.getItem('OAuth-token');
        const res = await makeCustomRequest({
            url : url,
            method : 'PATCH',
            body : {name : editEntry.name, address : editEntry.address}
        });
        console.log(res);
        if(res.success){
            editDialog.value= false;
            getTableData();
            toast.add({ severity: 'success', summary: 'Vendor updated successfully', life: 3000 });
        }
        else{
            toast.add({ severity: 'error', summary: 'Something went wrong', life: 3000 });
        }
...
const deleteEntrySingle =async () => {
    const url = `api/Masters/Vendor/${deleteEntry.id}`;
    const token = localStorage.getItem('OAuth-token');
    const res = await makeCustomRequest({
        url : url,
        method : 'DELETE',
    ...
    if(res.success){
        ...
        getTableData();
        toast.add({ severity: 'success', summary: 'Vendor deleted successfully', life: 3000 });
    }
    else{
        toast.add({ severity: 'error', summary: 'Something went wrong', life: 3000 });
    }
