<template>
    <div class="grid">
        <div class="col-12">
            <div class="card">
                <Toolbar class="mb-4">
                    <template v-slot:start>
                        <div class="my-2">
                            ...
                            <Button label="Export Products" icon="pi pi-download" class="mr-2" severity="success" @click="exportData('product')" />
                            <Button label="Export SKU" icon="pi pi-download" class="mr-2" severity="success" @click="exportData('sku')" />
                            <Button label="Import Data" icon="pi pi-download" class="mr-2" severity="success" @click="importDataShow" />
                        </div>
                    </template>
                </Toolbar>
                <DataTable
                    ...
                    <Column field="Parent" header="Category"  headerStyle="width:auto; min-width:10rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">Category</span>
                            {{ nameParentCatg(slotProps.data.category) }}
                        </template>
                    </Column>
                    <Column field="Parent" header="Created At"  headerStyle="width:auto; min-width:10rem;">
                        <template #body="slotProps">
                ...
                <div v-else class="flex items-center justify-center text-primary" style="justify-content : center">
                    <span>
                        <i class="pi pi-spin pi-spinner text-8xl"></i>
                    </span>
                </div>

                <Dialog v-model:visible="createDialog" position="top" :style="{ width: '90%' }" header="Create Product" :modal="true" class="p-fluid">
                    <div class="feild grid gap-1 mt-2">
                        ...
                        <div class="field col-3">
                            ...
                            <Dropdown v-model="makeEntry.category" :options="catgMast" optionLabel="name"  optionValue="id" placeholder="Select" />
                        </div>
                        <div class="field col-3">
                            <label for="name">MRP</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="makeEntry.mrp" required="true" autofocus />
                        </div>
                        <div class="field col-2">
                            <label for="name">Images</label>
                            <Button label="Upload" icon="pi pi-upload" class="mr-2" severity="success"  @click="imageUploaderProduct = true" />
                            <span class="text-primary"></span>
                            {{ makeEntry.images.length ? makeEntry.images.length + ' Selected' : '' }}
                        </div>
                        <div class="field col-3">
                            <label for="name">Tax %</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="makeEntry.tax" required="true" autofocus />
                        </div>
                        <div class="field col-3">
                            <label for="name">Discount (in RS)</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="makeEntry.discount" required="true" autofocus />
                        </div>
                        <div class="field col-3">
                            <label for="name">Weight (in grams)</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="makeEntry.weight" required="true" autofocus />
                        </div>
                        <div class="feild col-6">
                            <label for="name">Tags</label>
                            <MultiSelect
                            v-model="makeEntry.tags" :options="tagData"
                            option-label="name"
                            option-value="code"
                            @filter="filterInput"
                            ...
                            >
                            <template #filtericon>
                                <i @click="saveNewTag" class="pi pi-plus text-black	 p-icon p-multiselect-filter-icon"></i>
                            </template>
                            </MultiSelect>
                        </div>
                        <div class="feild col-5">
                            <label for="name">Vendor</label>
                            <Dropdown filter v-model="makeEntry.vendor" :options="vendorData" option-label="name" option-value="code" placeholder="Select" />
                        </div>
                    ...
                    <div class="feild grid gap-6 mt-2 mb-2">
                        <div v-for="list in homeListing " class="feild col-2">
                            <div class="field-checkbox mb-0">
                                <Checkbox id="checkOption1" name="option" :value="list.id" v-model="homeListingShow" />
                    ...
                    <template #footer>
                        ...
                        <Button label="Save" :loading="submitted" :disabled="submitted" icon="pi pi-check" class="mr-2" severity="success"  @click="saveEntry" />
                    </template>
                </Dialog>

                <Dialog v-model:visible="editDialog"position="top" :style="{ width: '90%' }" header="Edit product" :modal="true" class="p-fluid">
                    <div class="feild grid gap-1 mt-2">
                        <div class="field col-3">
                            <label for="name">Name <span class="text-red-400">*</span></label>
                        ...
                        <div class="field col-3">
                            <label for="name">Category <span class="text-red-400">*</span></label>
                            <Dropdown v-model="editEntry.category" :options="catgMast" optionLabel="name"  optionValue="id" placeholder="Select" />
                        </div>
                        <div class="field col-3">
                            <label for="name">MRP</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="editEntry.mrp" required="true" autofocus />
                        </div>
                        <div class="field col-2">
                            <label for="name">Images</label>
                            <Button label="Upload" icon="pi pi-upload" class="mr-2" severity="success"  @click="imageUploaderProductEdit = true" />
                            <span class="text-primary"></span>
                            {{ editEntry.images ? editEntry.images.length + ' Selected' : '' }}
                        </div>
                        <div class="field col-3">
                            <label for="name">Tax %</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="editEntry.tax" required="true" autofocus />
                        </div>
                        <div class="field col-3">
                            <label for="name">Discount (in RS)</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="editEntry.discount" required="true" autofocus />
                        </div>
                        <div class="field col-3">
                            <label for="name">Weight (in grams)</label>
                            <InputNumber  :minFractionDigits="0" :maxFractionDigits="5" id="name" v-model.trim="editEntry.weight" required="true" autofocus />
                        </div>
                        <div class="feild col-6">
                            <label for="name">Tags</label>
                            <MultiSelect
                            v-model="editEntry.tags" :options="tagData"
                            option-label="name"
                            ...
                            >
                            <template #filtericon>
                                <i @click="saveNewTagEdit" class="pi pi-plus text-black	 p-icon p-multiselect-filter-icon"></i>
                            </template>                            
                            </MultiSelect>
                        </div>
                        <div class="feild col-5">
                            <label for="name">Vendor</label>
                            <Dropdown filter v-model="editEntry.vendor" :options="vendorData" option-label="name" option-value="code" placeholder="Select" />
                        </div>                
                        <div class="field col-10">
                            <label for="name">Description</label>
                    ...
                    <div class="feild grid gap-6 mt-2 mb-2">
                        <div v-for="list in homeListing " class="feild col-2">
                            <div class="field-checkbox mb-0">
                                <Checkbox id="checkOption1" name="option" :value="list.id" v-model="homeListingShowEdit" />
                                <label for="checkOption1">{{ list.name }}</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <template #footer>
                        <Button label="Cancel" icon="pi pi-times" class="mr-2" severity="danger"  @click="hideDialog" />
                        <Button label="Save" icon="pi pi-check" :loading="submittededit" :disabled="submittededit" class="mr-2" severity="success"  @click="updateEntry" />
                    </template>
                </Dialog>                
                ...
                <Dialog v-model:visible="import_show" position="top" :style="{ width: '90%' }" header="Import Data" :modal="true" class="p-fluid">
                    <div class="feild grid gap-6 mt-2">
                        <div class="field col-2">
                            <label for="name">Sample Product</label>
                            <Button label="Export" icon="pi pi-download" class="mr-2" severity="success"  @click="exportSample('product')" />
                        </div>
                        <div class="field col-2">
                            <label for="name">Sample SKU</label>
                            <Button label="Export" icon="pi pi-download" class="mr-2" severity="success"  @click="exportSample('sku')" />
                        </div>
                        <div class="feild col-2">
                            <label for="name">Import Product</label>
                            <input class=" p-inputtext mt-2" @change="updateModel('product' , $event)" type="file">
                            <Button label="Import" icon="pi pi-upload"  class="mr-2 mt-2" severity="success"  @click="importFile('product')" />
                        </div>
                        <div class="feild col-2">
                            <label for="name">Import SKU</label>
                            <input class=" p-inputtext mt-2" type="file"  @change="updateModel('sku' , $event)">
                            <Button label="Import" icon="pi pi-upload" class="mr-2 mt-2" severity="success"  @click="importFile('sku')" />
                        </div>
                    </div>
                </Dialog>
            </div>
    ...
    <ImageUploader :visible="imageUploaderProduct" v-model="makeEntry.images" @close="imageUploaderProduct = false" ></ImageUploader>
    <ImageUploader :visible="imageUploaderProductEdit" v-model="editEntry.images" @close="imageUploaderProductEdit = false" ></ImageUploader>
</template>
...
const HighlightValues = ref([{ name: 'Yes' , value: 1 }, { name: 'No' , value : 0 }]);
...
const saveEntry = async () => {
    ...
    if(makeEntry.name != '' && makeEntry.category != ''){
        ...
        if(res.res){
            ...
            createNewTags.value = [];
            toast.add({ severity: 'success', summary: 'Product added successfully', life: 3000 });
        }
        else{
...
const exportSample =async (option) => {
    try {
        const config = useRuntimeConfig()
        const backendUrl = config.public.API_BASE_URL;
        const  url = option=='product' ? `${backendUrl}api/exportData?option=export_sample_product` : `${backendUrl}api/exportData?option=export_sample_sku`;
        const token = localStorage.getItem('OAuth-token');
        if(!token){
            navigateTo('/login');
        }
...
const importFile = async (option) => {
    ...
    const  url = option=='product' ? `${backendUrl}api/importData?option=import_product` : `${backendUrl}api/importData?option=import_sku`;
    ...
    if(response.data.value){
        if(response.data.value.success){
            ...
            toast.add({ severity: 'success', summary: 'Product uploaded successfully', life: 3000 });
        }
