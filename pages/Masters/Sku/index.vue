        ...
        <div class="col-12">
            <div class="card">
                ...
                <DataTable
                    ...
                    <Column field="Parent" header="Highlighted"  headerStyle="width:auto; min-width:10rem;">
                        <template #body="slotProps">
                            <span class="p-column-title">Highlighted</span>
                            {{ slotProps.data.is_highlighted ? 'Yes' : 'NO' }}
                        </template>
                    </Column>
                    <Column field="Discount" header="Discount"  headerStyle="width:auto; min-width:10rem;">
                        <template #body="slotProps">
                ...
                <Dialog v-model:visible="createDialog" :style="{ width: '450px' }" header="Create Product" :modal="true" class="p-fluid">
                        <div class="field">
                        ...
                        <div class="field">
                            ...
                            <InputText id="name" v-model.trim="makeEntry.discount" required="true" autofocus />
                        </div>
                        <div class="field">
                            <label for="name">Show in highlights <span class="text-red-400">*</span></label>
                            <SelectButton v-model="makeEntry.is_highlighted" :options="HighlightValues" optionLabel="name" optionValue="value" />
                        </div>
                    <template #footer>
                ...
                <Dialog v-model:visible="editDialog" :style="{ width: '450px' }" header="Create Role" :modal="true" class="p-fluid">
                    <div class="field">
                        ...
                        <div class="field">
                            <label for="name">Discount</label>
                            <InputText id="name" v-model.trim="editEntry.discount" required="true" autofocus />
                        </div>
                        <div class="field">
                            <label for="name">Show in highlights <span class="text-red-400">*</span></label>
                            <SelectButton v-model="editEntry.is_highlighted" :options="HighlightValues" optionLabel="name" optionValue="value" />
                        </div>
                    <template #footer>
...
const saveEntry = async () => {
    ...
    if(makeEntry.name != '' && makeEntry.category != '' && makeEntry.is_highlighted != ''){
        const url = "api/Masters/Products";
        const res = await makeCustomRequest({
            url : url,
            method : 'POST',
            body : {name : makeEntry.name, category : makeEntry.category , is_highlighted : makeEntry.is_highlighted , discount : makeEntry.discount}
        });
        if(res.res){
            createDialog.value= false;
            getTableData();
...
const updateEntry =async () => {
    ...
    if(editEntry.name != '' && editEntry.category != ''){
        ...
        const res = await makeCustomRequest({
            url : url,
            method : 'PATCH',
            body : {name : editEntry.name, category : editEntry.category , is_highlighted : editEntry.is_highlighted , discount : editEntry.discount}
        });
        if(res.res){
            editDialog.value= false;
            getTableData();
