...
const getTableData = async () => {
  ...
  const response = await makeCustomRequest({
    url: `api/topShareProduct?page=${page.value}`,
    method: "GET",
    query: {
      ...filters.value,
    },
...
<template>
  ...
  <DataTable v-if="data" class="mt-3" ref="table" :value="data" dataKey="id" :rows="10">
    ...
    <Column field="Type" header="Visits" headerStyle="width:auto; min-width:10rem;">
      <template #body="slotProps">
        <span class="p-column-title">phone</span>
        {{ slotProps.data.visited ? slotProps.data.visited : "" }}
      </template>
    </Column>
