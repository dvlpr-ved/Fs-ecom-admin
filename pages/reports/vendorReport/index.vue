...
const getTableData = async () => {
  ...
  const response = await makeCustomRequest({
    url: `api/vendorReport?page=${page.value}`,
    method: "GET",
    query: {
      ...filters.value,
    },
...
<template>
  ...
  <DataTable v-if="data" class="mt-3" ref="table" :value="data" dataKey="id" :rows="10">
    ...
    <Column field="Type" header="Vendor Name" headerStyle="width:auto; min-width:10rem;">
      <template #body="slotProps">
    ...
    <Column field="Type" header="Contact" headerStyle="width:auto; min-width:10rem;">
      <template #body="slotProps">
        <span class="p-column-title">user name</span>
        {{ slotProps.data.contact ? slotProps.data.contact : '' }}
      </template>
    </Column>    
    <Column field="Type" header="Address" headerStyle="width:auto; min-width:10rem;">
      <template #body="slotProps">
