<template>
    <div class="feild grid gap-6 mt-2">
      ...
      <div class="col text-align-right">
        <Button label="Add New" icon="pi pi-plus" class="mr-2 w-full" severity="success" @click="addRow"></Button>
      </div>
    </div>
      ...
      <div class="field col-4">
        <div class="feild grid">
          <div class="col-6">
            ...
            <InputText v-model.trim="localData[row].color" class="mt-2" required="true" autofocus />
          </div>
          <div class="col-6">
            <label for="color">Size <span class="text-red-400">*</span></label>
            <InputNumber  :minFractionDigits="0" class="mt-2" :maxFractionDigits="5" v-model.trim="localData[row].size" required="true" autofocus />
          </div>          
        </div>
      </div>
      ...
      <div class="field col-2">
        <div class="feild grid">
          <div class="col-6">
            ...
            <InputNumber  v-model.trim="localData[row].price" class="mt-2" required="true" autofocus />
          </div>
          <div class="col-6">
            <label for="color">Special Price  <span class="text-red-400">*</span></label>
            <InputNumber  v-model.trim="localData[row].price_subscribed" class="mt-2" required="true" autofocus />
          </div>
        </div>
      </div>
      <div class="field col">
        <label for="color">Images <span class="text-red-400">*</span></label>
        <Button label="Upload" icon="pi pi-upload"  severity="success" @click="UploaderVisible[row] = true" />
        {{ props.modelValue[row].images ? props.modelValue[row].images.length + " Uploaded" : '' }}
      </div>
      <div class="field col">
        <Button label="Copy" icon="pi pi-copy" class="mt-4" severity="success" @click="copyProduct(localData[row])" />
      </div>
      <div class="field col">
        <Button icon="pi pi-minus" class="mr-2 mt-4 w-9" severity="danger" @click="removeRow(row)"></Button>
      </div>
      <ImageUploader @close="UploaderVisible[row] = false" v-model="props.modelValue[row].images"  @hideDialog="HandlecloseProductUploader()" :visible="UploaderVisible[row]" ></ImageUploader>
    </div>
  </template>
  <script setup>
