export default  (args = []) => {
    ...
    const from_date = `${first_date.getDate().toString().padStart(2, '0')}-${(first_date.getMonth() + 1).toString().padStart(2, '0')}-${first_date.getFullYear()}`;
    const to_date = `${now.getDate().toString().padStart(2, '0')}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getFullYear()}`;
    let filters =  {
        'from_date' : from_date,
        'to_date' : to_date, 
    };
